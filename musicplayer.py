...
statusbar = ttk.Label(root, text="ENJOY BY HEARING SONGS", anchor=W, font='Arial 8 italic')
statusbar.pack(side=BOTTOM, fill=X)
statusbar1 = ttk.Label(root, text="SELECT AND ADD YOUR SONGS", anchor=W, font='Arial 8 italic')
...
root.style.configure('TButton', background='WHITE')
rtframe = Frame(root,bg="BLACK")
rtframe.pack(pady=30,padx=20)
topframe = Frame(rtframe,bg="BLACK")
topframe.pack()
root.style = ttk.Style()
#root.style.theme_use("clam")
root.style.configure('TLabel', background='SKY BLUE')
...
def play_music():
    ...
    if paused:
        ...
        try:
            ...
            tkinter.messagebox.showerror('File not found', 'Music player could not find the file. please try again!')
def stop_music():
    mixer.music.stop()
...
def mute_music():
    ...
    if muted:
        ...
        scale.set(0)
        muted = TRUE
middleframe = Frame(rtframe,bg="BLACK")
...
bottomframe = Frame(rtframe,bg="WHITE")
